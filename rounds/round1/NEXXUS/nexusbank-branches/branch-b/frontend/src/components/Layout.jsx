    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      <div style={{ background: PRIMARY, padding: '0 2rem', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span style={{ color: ACCENT, fontWeight: 700, fontSize: '1.2rem' }}>🏦 NexusBank</span>
          <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem' }}>Branch B — Bandra</span>
        </div>
        {user && (
      ...
      {user && (
        <div style={{ background: '#1a4fa0', padding: '0 2rem', display: 'flex', gap: '0.25rem' }}>
          {[['/dashboard','🏠 Dashboard'],[`/transactions/${user.account_no}`,'💳 Transactions'],['/profile','✏️ Profile'],['/search','🔍 Search'],['/docs','📄 Documents'],['/admin','⚙️ Admin']].map(([to, label]) => (
...
export function Btn({ children, variant = 'primary', ...props }) {
  const s = { primary: { background: PRIMARY, color: '#fff' }, danger: { background: '#e53e3e', color: '#fff' }, outline: { background: '#fff', border: '1px solid #e2e8f0', color: PRIMARY }, success: { background: '#38a169', color: '#fff' } }
